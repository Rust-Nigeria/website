...
pub fn EventCard(
    ...
    let (bg, text) = get_color_pair(index, 100);
    ...
    view! {
        <div class=cn!("h-full relative flex flex-col w-full border border-grey-20 bg-background-dark rounded-2xl text-left p-4", class)>
            ...
            <div style=format!("background-color: {}; color: {};", bg, text) class=" text-sm mt-4 px-2 py-1 font-medium rounded-lg w-fit">{date}</div>
                ...
                if event.speakers.is_empty() {
                    ...
                    Some(view! {
                        <div class="flex items-center gap-x-2 mt-2">
                            ...
                            <div class="flex">
                                {event.speakers.iter().enumerate().map(|(index, speaker)| view! {
                                    <img
                                        style=format!("transform: translateX(-{}%)", index * 30)
                                        class="size-8 border-2 border-white rounded-full"
                                        src=speaker.image.clone()
                                    />
                                }).collect_view()}
                            </div>
                        </div>
